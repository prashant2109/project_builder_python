...
class  PC_mgmg(object):
    ...
    def permut_operators(self, cmb_cnt):
        op_lst = ['+', '-', '*', '/']
        opr_permut_lst = []
        for permut_tup in product(op_lst, repeat=cmb_cnt):
            opr_permut_lst.append(permut_tup)
        return opr_permut_lst
    ...
    def read_all_modules(self, ijson):
        ...
        #permut_operators(self, cmb_cnt)
        check_val = 6
        for cmb_rng in range(2, len(res_lst)):
            for cmd_tup in permutations(res_lst, cmb_rng):
                cmb_cnt = len(cmd_tup) - 1
                opr_lst = self.permut_operators(cmb_cnt)
                for opr_tup in opr_lst:
                    opr_tup = opr_tup + ('', )
                    expression_lst = [sub[item] for item in range(len(opr_tup)) for sub in [list(cmd_tup), list(opr_tup)]] 
                    exp_str = ''.join(expression_lst[:-1])
                    eval_value = eval(exp_str)
                    if int(eval_value) == check_val:
                        print [cmd_tup, opr_tup, eval_value, exp_str]
        
    def compute_table_row_col_formula(self, ocmpany_id):
        json_file_path = '/mnt/eMB_db/company_management/{0}/json_files'
            
if __name__  ==  '__main__':
    p_Obj = PC_mgmg()
    ijson = {"project_id":"5"}
    print p_Obj.read_all_modules(ijson)
    #print p_Obj.permut_operators(1)
