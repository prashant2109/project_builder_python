...
class  PC_mgmg(object):
    def __init__(self):
        # row_id, module_name, description, module_key, fa_icon_info, doc_view, parent_key_info, level_id, user_select_flg, active
        # {'n':module_name, 'full_name':description, 'k':module_key, 'fa_icon':fa_icon_info, 'level_id':level_id, 'doc_view':bool(doc_view), 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}
        self.module_key_map  = {
                                0:'rid',
                                1:'n',
                                2:'full_name',
    ...
    def read_all_modules(self, ijson):
        ...
        for row in t_data:
            ...
            for ix, ks in enumerate(row):
                if not ks or ks == 'EMPTY':continue
